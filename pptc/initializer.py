from graph.potential import PotentialUtil, Potential
...
class Initializer(object):
    ...
    def initialize(join_tree):
        ...
        for clique in join_tree.get_cliques():
            ...
            nodes_in_clique = clique.nodes
            
            clique.size_pot = 1 
            
            join_tree.potentials[clique.id] = Potential() 
            
       
            for var in nodes_in_clique: 
                
                clique.size_pot*= len(join_tree.get_bbn_node(var.id).variable.values)  
                
                clique.scope.add(var) 
            
        for sep_set in join_tree.get_sep_sets():
            
            nodes_in_sep_set = sep_set.nodes
                        
            sep_set.size_pot = 1 
            
            join_tree.potentials[sep_set.id] = Potential() 

            
            for var in nodes_in_sep_set: 
                
                sep_set.size_pot*= len(join_tree.get_bbn_node(var.id).variable.values) 
                
                sep_set.scope.add(var) 
